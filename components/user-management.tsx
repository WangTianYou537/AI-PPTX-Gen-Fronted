...
export function UserManagement({ currentUser }: UserManagementProps) {
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    try {
      await createUser({ email, username, password, role, groupId, dailySlideLimit: slideLimit === "" ? undefined : Number(slideLimit) })
      toast.success("用户已创建")
      setEmail(""); setUsername(""); setPassword(""); setRole("user"); setSlideLimit("")
      await loadAll()
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
      <Card><CardHeader><CardTitle>新增用户</CardTitle><CardDescription>创建普通用户或管理员，可指定用户组和单独额度。</CardDescription></CardHeader><CardContent>
        <form onSubmit={handleCreate}><FieldGroup>
          ...
          <div className="grid grid-cols-1 gap-3"><Field><FieldLabel>页数/天</FieldLabel><Input type="number" min={0} value={slideLimit} onChange={(e) => setSlideLimit(e.target.value)} placeholder="继承" /></Field></div>
          ...
          {users.map((user) => (
            <div key={user.id} className="flex flex-col gap-3 rounded-lg border p-3">
              ...
              <div className="grid gap-2 md:grid-cols-3">
                ...
                <Select value={user.dailySlideLimit == null ? inherit : String(user.dailySlideLimit)} onValueChange={(value) => patch(user, { dailySlideLimit: value === inherit ? null : Number(value) }, "页数额度已更新")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value={inherit}>页数继承组</SelectItem>{[0,10,30,50,100,200,500].map((n) => <SelectItem key={n} value={String(n)}>{n} 页/天</SelectItem>)}</SelectContent></Select>
              </div>
              <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => patch(user, { disabled: !user.disabled }, user.disabled ? "用户已启用" : "用户已禁用")} disabled={user.id === currentUser.id}>{user.disabled ? "启用" : "禁用"}</Button><FieldDescription>{user.dailySlideLimit == null ? "页数额度继承用户组" : "已设置单用户页数额度覆盖"}</FieldDescription></div>
            </div>
          ))}
        </div>
      </CardContent></Card>
